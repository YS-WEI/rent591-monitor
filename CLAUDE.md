...
6. **同一物件被多仲介重複刊登（可達 11 筆）＋「24 號段」死連結**：BFF 會把同一間房由
   不同仲介以不同 `houseid` 各刊一次。其中 id ≥ 24,000,000（「24 號段」）者為代銷/同步
   餵入來源，**sale.591 沒有詳情頁**，`/home/house/detail/2/{id}.html` 一律 404；正常
   中古屋在 20–21 號段、連結正常（此號段分界目前 100% 準，591 若改 id 配置需重驗）。
   這些 24 號段不一定是開放式/建案，常是正常 4 房的重複刊登，且會輪替出現。
   對策（`sale_parser.dedupe_sale_units`）：丟棄 24 號段死連結，再以（社區/坪數/樓層/
   房數/總價）模糊鍵同案去重，代表取**最小 houseid**（跨輪最穩定，減少假新增/假下架）。
